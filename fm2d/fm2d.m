...
% 输入的sourcePoints dy dx都是现实物理量
% sourcePoints (2, NumOfSPs)
%% Initialization
...
for iter = 1:NumOfSPs
    ...
    % CPnode就是CP的物理坐标，CP是F坐标系的坐标，CP的作用是什么？
    % indice是二维坐标变成一维的index
	CPnode = getNodes(CP,[dy dx]);
	CPInd = sub2ind(CPnode(1),CPnode(2),m);

	% Calculate travel time and freeze
    % getPoints让CPnode从物理坐标变回坐标系坐标的时候会有误差？？？
    % 这一步在干嘛，为什么要物理坐标和坐标系坐标来回换啊？
